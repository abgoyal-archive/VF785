...
# -----------------------------------------------------------------
#   End. Modified by jinming.xiang for FR 442297
# -----------------------------------------------------------------
...
if($EMMC_SUPPORT eq "yes"){
	...
	{
    #susheng.ye change for eng and user target buid using the same partition table 20130427 begin 
    #if(!defined $TARGET_BUILD_VARIANT || $TARGET_BUILD_VARIANT eq ""){
     # $SHEET_NAME = $SHEET_NAME . " eng";
    #}else{
     # if($TARGET_BUILD_VARIANT eq "eng"){
      ...
      #}
    #}
   #susheng.ye change for eng and user target buid using the same partition table 20130427 end 
	}
}else{
