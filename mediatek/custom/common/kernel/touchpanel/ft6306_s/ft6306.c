...
static unsigned char CTPM_FW_CAMRY_TRULY[]=
{
	...
	#include "YarisS_ID80_V14_20130530_app.i"
};

static unsigned char CTPM_FW_CAMRY_MUTTO[]=
{
	//#include "ft_app_camry_mutto.i"
	#include "YarisS_ID80_V15_20130603_app.i"
};

static unsigned char CTPM_FW_YARIS_EACH[]=
{
...
static unsigned char CTPM_FW_YARIS_JUNDA[]=
{
	//#include "Ver0x0B_Beetle_FT6306_0x85_20130222_app.i"
	#include "YarisS_ID85_V11_20130710_app.i"
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if(tp_vendor == 0x5a){
		...
		pbt_buf = CTPM_FW_YARIS_EACH;
		ui_sz = sizeof(CTPM_FW_YARIS_EACH);
	}
	else if(tp_vendor == 0x85){
		pbt_buf = CTPM_FW_YARIS_JUNDA;
		ui_sz = sizeof(CTPM_FW_YARIS_JUNDA);
	}
...
unsigned char fts_ctpm_get_i_file_ver(void)
{
	...
	if (ui_sz > 2)
	{
		if(tp_vendor == 0x5a)
			...
			return CTPM_FW_YARIS_EACH[ui_sz - 2];
		else if(tp_vendor == 0x85)
			return CTPM_FW_YARIS_JUNDA[ui_sz - 2];
	}
	else
		//TBD, error handling?
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char version_list_camry_each[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15,0x16};	//Keep the ft6306 tp firmware old version list that allows to be updated.
